...
import ru.spb.ipo.generator.image_generator.cdsl.model.Card;
...
public class CardParser {
    ...
    public String parse(String taskTitle, String taskText) {
        ...
        try {
            ...
            */
            ...
            // Просто игнорируем ошибки при парсинге
        }
    ...
    /**
     * Парсит карту из текстового паттерна
     */
    private CardInfo parseCardFromPattern(String pattern) {
        if (pattern.contains("туз")) {
            if (pattern.contains("черв")) return new CardInfo("ACE", "HEARTS");
        } else if (pattern.contains("король")) {
            if (pattern.contains("пик")) return new CardInfo("KING", "SPADES");
        } else if (pattern.contains("дама")) {
            if (pattern.contains("бубен")) return new CardInfo("QUEEN", "DIAMONDS");
        } else if (pattern.contains("валет")) {
            if (pattern.contains("треф")) return new CardInfo("JACK", "CLUBS");
        } else if (pattern.contains("десятка")) {
            if (pattern.contains("черв")) return new CardInfo("10", "HEARTS");
        } else if (pattern.contains("девятка")) {
            if (pattern.contains("бубен")) return new CardInfo("9", "DIAMONDS");
        } else if (pattern.contains("восьмерка")) {
            if (pattern.contains("пик")) return new CardInfo("8", "SPADES");
        } else if (pattern.contains("семерка")) {
            if (pattern.contains("треф")) return new CardInfo("7", "CLUBS");
        }
        return null;
    }
